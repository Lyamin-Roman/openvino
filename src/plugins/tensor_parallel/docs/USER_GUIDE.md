...
`TENSOR_PARALLEL` is an OpenVINO meta-device that runs a single
...
- OpenVINO built with `-DENABLE_TENSOR_PARALLEL=ON` (the default).
...
auto compiled = core.compile_model(model, "TENSOR_PARALLEL", {
    {"TENSOR_PARALLEL_DEGREE", uint32_t(2)},
    // Optional: pin to specific devices.  Default: GPU.0, GPU.1, ...
    // {"TENSOR_PARALLEL_DEVICES", std::vector<std::string>{"GPU.0", "GPU.1"}},
...
compiled = core.compile_model(model, "TENSOR_PARALLEL", {
...
|------------------------------|-------------------------------|-------------------------------|-------------|
| `TENSOR_PARALLEL_DEGREE`     | `uint32_t`                    | `2`                           | Number of ranks. Must equal `len(TENSOR_PARALLEL_DEVICES)` when both are set. |
| `TENSOR_PARALLEL_DEVICES`    | `std::vector<std::string>`    | `["GPU.0", "GPU.1", ...]`     | Explicit per-rank devices. |
...
- **Divisibility.** `num_heads`, `num_kv_heads`, and `intermediate_size`
  must each be divisible by `TENSOR_PARALLEL_DEGREE`. Non-divisible
...
### `TENSOR_PARALLEL` not in `core.get_available_devices()`

The plugin failed to load. Check:

- The shared object exists at `bin/intel64/Release/libopenvino_tensor_parallel_plugin.so`.
- It is registered in `plugins.xml` (the build wires this up
  automatically — a stale `plugins.xml` from a previous build with
  `-DENABLE_TENSOR_PARALLEL=OFF` is a common cause; rebuild from
  scratch).
- `LD_LIBRARY_PATH` includes the runtime libs and the L0 loader is
  reachable.

### `[TENSOR_PARALLEL] Need at least 2 devices, got 1`
