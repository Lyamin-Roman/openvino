...
The Tensor Parallel (`TENSOR_PARALLEL`) plugin is an OpenVINO meta-plugin that
...
┌──────────────────────────────────────────────────────────────┐
│                     User Application                         │
│   core.compile_model(model, "TENSOR_PARALLEL", config)       │
...
```
src/plugins/tensor_parallel/
├── CMakeLists.txt
├── docs/                                  ← this directory
├── include/tensor_parallel/
│   ├── op/                                ← public op headers (registered in
│   │                                        a private "tp_internal_opset")
